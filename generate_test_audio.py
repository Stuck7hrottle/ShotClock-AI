...
def create_test_wav(
    filename, shot_times, amplitude=0.8, sr=48000, noise_floor=0.01, echoes=None, interference=None
):
    """
...
create_test_wav(
    "test_audio/05_interference.wav", [0.5 + i * 0.1 for i in range(10)], interference=bg_shots
)

# 6. Heavy Echoes: Test echo_window_ms (default 45ms)
# Each shot followed by a strong echo at 35ms
create_test_wav(
    "test_audio/06_echo_chamber.wav", [0.5 + i * 0.15 for i in range(5)], echoes=[(35, 0.5)]
)
...
create_test_wav(
    "test_audio/10_noisy_env.wav",
    [0.5 + i * 0.1 for i in range(10)],
    amplitude=0.15,
    noise_floor=0.08,
)
...
multi = (
    [0.5 + i * 0.1 for i in range(3)]
    + [1.5 + i * 0.1 for i in range(3)]
    + [2.5 + i * 0.1 for i in range(3)]
)
